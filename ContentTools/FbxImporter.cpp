...
namespace primal::tools {
	...
	bool fbx_context::get_mesh_data(FbxMesh* fbx_mesh, mesh& m) {
		...
		for (s32 i{ 0 }; i < num_indices; ++i) {
			...
			// did we encounter this vertex before? 
			// if so, just add its index, if not, add the vertex and a new index
		...
		// NOTE: it's ok if we don't have an UV set. For example, some emissive objects don't need an UV map
