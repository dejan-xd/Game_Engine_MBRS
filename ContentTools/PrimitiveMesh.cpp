...
namespace primal::tools {
	namespace {
		...
		mesh create_uv_sphere(const primitive_init_info& info) {
			...
			const u32 phi_count{ clamp(info.segments[axis::x], 3u, 64u) };		// we can't have a circle that has less than 3 vertical segments
			const u32 theta_count{ clamp(info.segments[axis::y], 2u, 64u) };	// we can't have a circle that has less than 2 horizontal segments
			const f32 theta_step{ pi / theta_count };
			const f32 phi_step{ two_pi / phi_count };
			const u32 num_indices{ 2 * 3 * phi_count + 2 * 3 * phi_count * (theta_count - 2) };	// top and bottom cap + middle part
	...
	EDITOR_INTERFACE void CreatePrimitiveMesh(scene_data* data, primitive_init_info* info) {
		...
		scene scene{}; // not the same as in level editor, it is a collection of all the geometry that's in the file
