...
namespace primal::graphics::d3d12::d3dx {
	...
	ID3D12Resource* create_buffer(const void* data, u32 buffer_size, bool is_cpu_accessible /* = false */, D3D12_RESOURCE_STATES state /* = D3D12_RESOURCE_STATE_COMMON */,
		...
		if (heap) {
			...
			DXCall(core::device()->CreateCommittedResource(is_cpu_accessible ? &heap_properties.upload_heap : &heap_properties.default_heap,
				D3D12_HEAP_FLAG_NONE, &desc, resource_state, nullptr, IID_PPV_ARGS(&resource)));
		}

		if(data) {
			// If we have initial data which we'd like to be able to change later, we set is_cpu_accessible to true.
			// If we only want upload some data once to be used by the GPU, then is_cpu_accessible should be set to false.
			if (is_cpu_accessible) {
				// NOTE: range's Begin and End fields are set to 0, to indicate that the CPU is not reading any data (i.e. write-only)
				...
				// We need an upload context here. This is for uploading resources that are only accessed by the GPU.
