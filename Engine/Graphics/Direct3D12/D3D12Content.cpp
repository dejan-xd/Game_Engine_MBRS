...
namespace primal::graphics::d3d12::content {
	...
	namespace {
		...
		id::id_type create_pso_if_needed(const u8* const stream_ptr, u64 aligned_stream_size, [[maybe_unused]] bool is_depth) {
			...
			{ // Lock scope to heck if PSO already exists
	...
	void shutdown() {
		// NOTE: We only release data that were created as a side-effect to adding resources, which the user of this module has no control over. 
		//		 The rest of data should be released by the user, by calling "remove" functions, prior to shutting down the renderer.
	...
	namespace submesh {
		...
		//		u8 positions[sizeof(f32) * 3 * vertex_count],			// sizeof(positions) must be a multiple of 4 butes. Pad if needed.
		//		u8 elements[sizeof(element_size) * vertex_count],		// sizeof(elements) must be a multiple of 4 butes. Pad if needed.
		//		u8 indices[index_size * index_count]
		//
		// Remarks:
		//  - Advances the data pointer
		//  - Position and element buffers should be padded to be a multiple of 4 bytes in length.
		//	  This 4 bytes is defined ad D3D12_STANDARD_MAXIMUM_ELEMENT_ALIGNMENT_BYTE_MULTIPLE
