...
namespace primal::graphics::d3d12::delight {
	...
	namespace {
		...
		struct culling_parameters {
			...
			// NOTE: initialize has_lights with 'true' so that the culling shader is run at least once in order to clear the buffer.
	...
	void cull_lights(id3d12_graphics_command_list* const cmd_list, const d3d12_frame_info& d3d12_info, d3dx::d3d12_resource_barrier& barriers) {
		...
		if (d3d12_info.surface_width != culler.view_width || d3d12_info.surface_height != culler.view_height ||
		...
		// NOTE: We update culler.has_lights after this statement, so the light culling shader will run once to clear the buffers when there're no lights
