...
namespace primal::graphics::d3d12::upload {
	...
	namespace {
		...
		u32 get_available_upload_frame() {
			...
			// None of the frames were done uploading. We're the only thread here so we can iterate through frames until we find one that is ready.
	...
	d3d12_upload_context::d3d12_upload_context(u32 aligned_size) {

		{
			...
			// Before unlocking, we prevent other threads from picking this frame by making is_ready return false
